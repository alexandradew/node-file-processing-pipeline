...
Tests run against the real Postgres/MinIO from `docker compose`, so the infra needs to be up first

```bash
npm run test --workspace=api
```

The e2e test (`api/src/e2e/upload-flow.test.js`) starts its own Express server on port `3000` to actually receive the MinIO webhook, so you can't run `npm run dev:api` at the same time (port conflict)
